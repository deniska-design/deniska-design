...
{    
    ...
    while (true)
    {
        ...
        {
            printf("пришло сообщение от сервера\n");
            ...
            {
                printf("нового года не будет. идите нахуй\n");
                return 0;
            }
        }
        ...
        if (messangeFor != "\0")
        {
            ...
            {
                printf("пришло время отправить сообщение серверу\n");
...
int SendMessange(int recipSock, const char *&messange)
{
...
int ReadMessange(int rd, char *messangeFrom)
{
	int ReadBytes;
	printf("пришло сообщение от игрока\n");
	if (0 > (ReadBytes = read(rd, messangeFrom, sizeof(messangeFrom))))
	{
		printf("ошибка чтения данных:%d\n", errno);
		return(-1);
	}else if(ReadBytes > 0)
	{
		printf("сообщение от игрока: %s\n", messangeFrom);
		return 1;
	}else return 0;
}
int main()
{   
    char messangeFrom[4][256];
    const char *messangeFor[4];
    int sd, MaxD, SelRes, Res;
    int playerCount = 0;
    int pd[4];
	...
	//начало работы с игроками на прямую:
        ...
        for (int i = 0; i < playerCount; i++)
        {
            ...
            {
		    if((Res = ReadMessange(pd[i], messangeFrom[i])) == 1)
            {
				printf("%s\n", messangeFrom[i]);
				messangeFor[i] = "siski dhh\n";
		    }else if(Res == 0)
		    {
